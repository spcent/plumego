...
#### 命令 #5-#11: 其他已实现命令（简要）
- `plumego dev` - 双进程开发模式（Dashboard + 热重载），支持 `--no-reload`、`--build-cmd`、`--run-cmd`
- `plumego routes` - AST 路由分析，支持 `--method`/`--pattern`/`--middleware` 过滤
- `plumego build` - 封装 `go build`，支持 `--trimpath`、`--race`、`--tags`、`--ldflags`
- `plumego test` - 封装 `go test -json`，支持 `--cover`/`--bench`/`--timeout`/`--tags`
- `plumego inspect` - 运行时检查（health/metrics/routes/config/info）
- `plumego migrate` - 迁移管理（create/up/down/status），需要 `--driver` + `--db-url`
- `plumego version` - 版本、commit、构建时间与 Go 版本信息

**示例:**
```bash
plumego dev --dashboard-addr :9999
plumego routes --method GET --format json
