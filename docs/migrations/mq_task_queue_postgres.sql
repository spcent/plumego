-- Durable task queue schema (PostgreSQL)
...
CREATE INDEX IF NOT EXISTS idx_mq_tasks_ready
  ON mq_tasks (status, available_at, priority, created_at);

CREATE INDEX IF NOT EXISTS idx_mq_tasks_lease
  ON mq_tasks (lease_until, lease_owner);

CREATE INDEX IF NOT EXISTS idx_mq_tasks_tenant_topic
  ON mq_tasks (tenant_id, topic);

CREATE UNIQUE INDEX IF NOT EXISTS uq_mq_tasks_dedupe
