...
| `x/frontend` | experimental | medium | frontend | Explicit static or embedded asset serving | docs:primer; coverage:asset-serving | `go test -timeout 20s ./x/frontend/...` | Hardened asset-serving coverage; needs exported API snapshot from current registrar/config surface, two-release history, and owner sign-off before promotion |
