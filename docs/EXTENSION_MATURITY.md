...
| `x/data` | experimental | medium | persistence | `docs/modules/x-data/README.md` before subordinate data packages | `go test -timeout 20s ./x/data/...` | [maturity note](extension-evidence/x-data.md): sub-surface inventory selects `x/data/file` and `x/data/idempotency` as possible future beta targets; topology surfaces remain experimental |
...
| `x/discovery` | experimental | medium | edge | Caller-selected discovery backend | `go test -timeout 20s ./x/discovery/...` | [maturity note](extension-evidence/x-discovery.md): core contract and static backend are possible future beta targets; Consul/Kubernetes/etcd need backend release observation |
