...
| `x/frontend` | experimental | medium | frontend | Explicit static or embedded asset serving | docs:primer; coverage:asset-serving; hardening:directory-safety/precompressed/negotiation | `go test -timeout 20s ./x/frontend/...` | [beta evidence](extension-evidence/x-frontend.md): current-head snapshot is development-only; release history, release-backed API snapshots, and owner sign-off remain missing |
