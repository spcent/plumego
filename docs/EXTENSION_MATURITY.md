...
promoting the parent family. For `x/ai/provider` and `x/ai/session`, the new
subpackage manifests own package-level owner, risk, and validation metadata;
`x/ai/module.yaml` remains the family-tier index, and the evidence docs remain
