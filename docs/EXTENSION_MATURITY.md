...
| `x/data` | experimental | medium | persistence | `docs/modules/x-data/README.md` before subordinate data packages | `go test -timeout 20s ./x/data/...` | [maturity note](extension-evidence/x-data.md): topology-heavy package set; needs feature-level API segmentation and release observation |
...
| `x/discovery` | experimental | medium | edge | Caller-selected discovery backend | `go test -timeout 20s ./x/discovery/...` | [maturity note](extension-evidence/x-discovery.md): Kubernetes/etcd backends need release observation and backend contract freeze |
...
| `x/messaging` | experimental | medium | messaging | App-facing messaging entrypoint before queue/pubsub primitives | `go test -timeout 20s ./x/messaging/...` | [maturity note](extension-evidence/x-messaging.md): evaluate after subordinate mq/pubsub/scheduler/webhook contracts settle |
