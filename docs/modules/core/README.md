...
- keep `core` as the first-party router owner: route wiring goes through `App.AddRoute` / `App.Get` / `App.Post` and `App.AddRouteWithName` for named routes; reverse URL lookup goes through `App.URL(...)`, not raw router replacement or mutation
