...
- extend `docs/concepts/x-tenant-blueprint.md` only as implemented behavior changes land
...
- `docs/start/getting-started.md` matches the actual API surface
...
- keep `docs/start/getting-started.md` and module primers aligned with `reference/standard-service`
...
- `docs/reference/extension-stability-policy.md` defines the `experimental` → `beta` → `ga`
...
- `docs/concepts/extension-maturity.md` provides the human-readable `x/*` maturity
...
- `x/rest`, `x/websocket`, `x/gateway`, and `x/observability` promoted to `beta`
  ...
  `docs/evidence/extension/snapshots/`, owner sign-off recorded, all blockers
  cleared in `specs/extension-beta-evidence.yaml`
- `docs/evidence/extension/BETA_EVIDENCE_TEMPLATE.md` updated to reflect the
...
- `docs/reference/deprecation.md` beta extension table updated to include all promoted families
...
- `docs/evidence/benchmarks/README.md`: benchmark methodology, medians vs. Chi/Gin/Echo,
  net/http compatibility cost explanation
- `docs/start/troubleshooting.md`: 14-category onboarding troubleshooting guide
- `docs/start/getting-started_CN.md`: Chinese onboarding guide (complete)
...
- `docs/start/adoption-path.md`: gRPC/RPC and OpenAPI entries added
...
- keep the first-user path aligned with `docs/start/adoption-path.md`
...
- enforce scaffold/reference drift through `docs/reference/scaffold-reference-contract.md`
