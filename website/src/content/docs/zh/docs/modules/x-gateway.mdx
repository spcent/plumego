...
> **Beta** — API 在 minor 版本内保持稳定。采用前请查看[发布策略](/zh/docs/release-posture)和[扩展成熟度](/zh/docs/concepts/extension-maturity)。
...
- `x/gateway/cache` — gateway edge 层 HTTP 响应缓存
- `x/gateway/discovery` — 服务实例解析
- `x/gateway/ipc` — 进程间传输
- `x/gateway/protocol` — 协议适配器注册表（gRPC、GraphQL 转码）
- `x/gateway/protocolmw` — 代理管道中协议感知的 middleware
- `x/gateway/transform` — 请求/响应 header、查询参数与 body 变换链

## 稳定性与采用

当兼容性很重要时，gateway 和 edge transport 行为应通过应用本地配置隔离。生产中采用 proxy、rewrite、balancer、discovery 或 IPC API 前，请检查 `x/gateway/module.yaml`、[发布策略](/zh/docs/release-posture) 和 [扩展成熟度](/zh/docs/concepts/extension-maturity)。

## 什么时候从这里开始

- 你正在构建或修改反向代理行为（`proxy.go`）
- 你正在配置 gateway 路由规则或路径重写（`rewrite.go`）
...
- 你正在通过代理管道适配 gRPC 或 GraphQL 上游协议（`protocol/`、`protocolmw/`）
- 你正在 gateway edge 层组合 header、查询参数或 body 变换（`transform/`）
...
5. `x/gateway/rewrite.go`（如果涉及路径重写）
...
Gateway 与 edge transport 是"路由"（属于 `router`）与"代理"（属于这里）之间混淆最容易导致边界漂移的地方。`x/gateway` 是所有 6 个下级包的守门人：在直接打开 `cache`、`discovery`、`ipc`、`protocol`、`protocolmw` 或 `transform` 之前，始终先从这里开始。这种间接层使面向应用的表面保持稳定，即使下级包内部在演进。
