...
`x/rest` 是扩展表面，不属于最小稳定根路径。生产路径采用前，请检查
`x/rest/module.yaml`、[发布策略](/zh/docs/release-posture) 和
[扩展成熟度](/zh/docs/concepts/extension-maturity)。如果兼容性很重要，建议用应用本地 controller 或 interface 隔离。
