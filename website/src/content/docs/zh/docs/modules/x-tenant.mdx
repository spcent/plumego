...
> **Beta** — API 在小版本内保持稳定。采用前请查看[发布策略](/zh/docs/release-posture)和[扩展成熟度](/zh/docs/concepts/extension-maturity)。
...
租户隔离是安全敏感能力，但这并不意味着 `x/tenant` 是稳定根。采用前请检查
`x/tenant/module.yaml`、[发布策略](/zh/docs/release-posture) 和
[扩展成熟度](/zh/docs/concepts/extension-maturity)。生产服务中，建议用应用本地 interface 隔离租户策略和存储选择，降低升级影响。
...
- `quota.Middleware` 与 `ratelimit.Middleware`：处理租户范围内的预算与速率反馈
- `session.SessionCheck` 与 `session.NewJWTStateStore`：在每个请求上检查租户会话状态
