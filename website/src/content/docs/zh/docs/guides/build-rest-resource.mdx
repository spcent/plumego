...
`x/rest` 在当前发布矩阵中仍属于 experimental 扩展家族。生产路径中建议通过应用本地 controller 或 adapter 隔离它，并在假设 API 或配置兼容性已冻结前先查看[发布策略](/zh/docs/release-posture)和[扩展成熟度](/zh/docs/concepts/extension-maturity)。
...
| 生产升级风险不清楚 | 检查 `x/rest/module.yaml`、[发布策略](/zh/docs/release-posture)，并用应用测试固定查询语义 |
