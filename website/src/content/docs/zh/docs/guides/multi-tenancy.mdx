...
`x/tenant` 是 experimental 且高风险的扩展家族，因为租户错误可能直接演变成隔离失败。租户 wiring 必须保持显式，并在 resolution、policy、quota、session 错误上 fail closed；在把它当作冻结的生产契约前，请先查看[发布策略](/zh/docs/release-posture)和[扩展成熟度](/zh/docs/concepts/extension-maturity)。
