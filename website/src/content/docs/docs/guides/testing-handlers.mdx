...
[`reference/standard-service/internal/handler/api.go`](https://github.com/spcent/plumego/blob/main/reference/standard-service/internal/handler/api.go) shows handler structs with injected dependencies that follow the same shape covered in this guide. The test files alongside it demonstrate the `httptest.NewRecorder` pattern in practice.

## Read next

- [Handle Errors](/docs/guides/handle-errors)
- [Error Reference](/docs/reference/errors) — error type catalog for asserting error responses in tests
