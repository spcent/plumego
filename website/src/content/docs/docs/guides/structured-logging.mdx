...
`NewLogger` returns a `log.StructuredLogger`. It writes human-readable text by default. For JSON output, use `NewLoggerWithConfig`:

```go
logger := plumelog.NewLoggerWithConfig(plumelog.LoggerConfig{
