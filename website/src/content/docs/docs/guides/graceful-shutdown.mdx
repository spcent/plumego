...
[`reference/standard-service/main.go`](https://github.com/spcent/plumego/blob/main/reference/standard-service/main.go) shows the complete graceful-shutdown wiring — signal context, `ListenAndServe` error channel, and `app.Shutdown` with a deadline — in a runnable service.

## Read next

- [core API Reference](/docs/reference/api-core) — `App.Prepare`, `App.Server`, and `App.Shutdown` signatures
