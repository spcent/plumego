...
- Implementing a `Repository[T]` for a resource
- Wiring a `DBResourceController[T]` to routes
- Parsing query parameters (page, sort, filter)
- Writing a custom controller when the generated one is not enough
