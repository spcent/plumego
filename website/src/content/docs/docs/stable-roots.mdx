...
| file contracts, cache primitives, KV helpers, idempotency contracts, and DB helper boundaries | [Store Primer](/docs/modules/store) | it gives the clearest concrete split between stable persistence primitives and `x/data`, `x/data/cache`, `x/fileapi`, or `x/tenant` work |
