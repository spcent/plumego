...
Returns the value of path parameter `key` from the matched route. Returns `""` when the key is absent. Prefer `contract.Param` or `contract.MustParam` from handler code to keep `router` out of handler imports.
...
Path parameters are extracted with `router.Param(r, "id")` or `contract.MustParam(r, "id")`.
