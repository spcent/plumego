...
| --- | --- |
| Whether Plumego fits my project | [Fit check](#is-plumego-the-right-fit-for-my-team) |
| How to build a REST API | [REST API](#how-do-i-build-a-rest-api-with-plumego) |
| How to add authentication | [JWT Auth](#how-do-i-add-authentication-to-my-routes) |
| How to connect a database | [Database](#how-do-i-connect-a-database) |
| How Plumego compares to Gin/Echo/Chi | [Comparison](#how-do-plumego-and-gin--echo--chi-compare) |
| Whether I can use it in production now | [Production readiness](#can-i-use-plumego-in-production-right-now) |
| Router performance overhead | [Benchmarks](#what-is-the-routers-performance-overhead-vs-stdlib) |
| Stable root vs x/* decision | [Module decision](#how-do-i-decide-between-a-stable-root-and-x) |
| Custom error format | [Custom errors](#how-do-i-customize-the-error-response-format) |
| Explicit DI rationale | [DI rationale](#why-does-plumego-use-explicit-di-instead-of-a-service-locator) |

---

## Getting Started

### Is Plumego the right fit for my team?

Read [When Not to Use Plumego](/docs/when-not-to-use) before committing to technical evaluation. That page lists the trade-offs Plumego makes deliberately — and the conditions under which a different tool is the better choice. The honest answer matters more than the feature list.
...
See [Stability](/stability) for the four-tier overview and [Releases](/releases) for the full module-by-module support matrix.
...
The overhead covers per-request context injection (480 B, 5 allocs), parameter map construction, and middleware chain wiring. For typical handlers spending 1 ms–100 ms on I/O, router cost is 0.001%–0.1% of total request time. See the [full benchmark table on Releases](/releases) for all nine scenarios and methodology.
...
| Question | Page |
| --- | --- |
| Exact function signatures for `core`, `contract`, `router` | [API Quick Reference](/docs/reference) |
| All error types with HTTP status and remediation | [Error Reference](/docs/reference/errors) |
| What does the reference service look like? | [Reference App](/docs/reference-app) |
| Which APIs are stable vs experimental? | [Release Posture](/docs/release-posture) |
| When should I NOT use Plumego? | [When Not to Use](/docs/when-not-to-use) |
