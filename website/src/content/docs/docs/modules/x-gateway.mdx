...
> **Beta** — API is stable within a minor version. Check [Release Posture](/docs/release-posture) and [Extension Maturity](/docs/concepts/extension-maturity) before adopting in production.
...
`x/gateway` is the canonical and only app-facing entrypoint for gateway and edge transport work in Plumego. Its two subordinate packages — `x/gateway/discovery` (service instance resolution) and `x/gateway/ipc` (inter-process transport) — are reached through `x/gateway` first, not opened directly.
