...
`Prepare` freezes the route table and builds the handler chain. `Server` returns the `*http.Server`. `Shutdown` drains in-flight connections and respects the context deadline. See [Graceful Shutdown](/docs/guides/graceful-shutdown) for the complete pattern including resource cleanup.
...
- [core API Quick Reference](/docs/reference/api-core) — complete function signatures and type definitions
