...
// Uses UTC by default.
type Config struct {
	Enabled  *bool         `json:"enabled,omitempty"`  // Whether the maintenance period is enabled. Enabled by default if nil.
	Start    string        `json:"start,omitempty"`    // Time at which the maintenance period starts (e.g. 23:00)
	Duration time.Duration `json:"duration,omitempty"` // Duration of the maintenance period (e.g. 4h)
	Timezone string        `json:"timezone,omitempty"` // Timezone in string format which the maintenance period is configured (e.g. America/Sao_Paulo)
...
// UnmarshalJSON accepts Duration as either a duration string ("4h") or numeric
...
func (c *Config) UnmarshalJSON(data []byte) error {
	...
	case string:
		...
		if err != nil {
			return fmt.Errorf("maintenance duration: %w", err)
		}
		c.Duration = d
	case float64:
		c.Duration = time.Duration(v)
	default:
		return fmt.Errorf("maintenance duration: unsupported type %T", v)
	}
	return nil
}
