// Package mongohistory stores MongoDB aggregation pipeline history per connection.
...
// Entry is a single recorded aggregation pipeline execution.
type Entry struct {
	ID          string    `json:"id"`
...
// Store persists aggregation pipeline history in a KV store.
type Store struct {
	kv *kvstore.KVStore
}

// NewStore creates a mongohistory Store.
...
func key(connID string) string { return fmt.Sprintf("mongohistory:%s", connID) }
