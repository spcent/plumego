// Package eshistory stores Elasticsearch search DSL history per connection.
package eshistory
...
// Entry is a single recorded search DSL execution.
type Entry struct {
	ID          string    `json:"id"`
	ConnID      string    `json:"conn_id"`
	Index       string    `json:"index"`
	DSL         string    `json:"dsl"` // raw JSON string
...
// Store persists search DSL history in a KV store.
type Store struct {
	kv *kvstore.KVStore
}

// NewStore creates an eshistory Store.
...
func key(connID string) string { return fmt.Sprintf("eshistory:%s", connID) }
