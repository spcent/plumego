...
type redisExportPayload struct {
	Version   string            `json:"version"`
	DB        int               `json:"db"`
	Pattern   string            `json:"pattern"`
	Items     []redisExportItem `json:"items"`
	Limit     int               `json:"limit"`
	Truncated bool              `json:"truncated"`
}

type redisExportItem struct {
	Key   string `json:"key"`
	Type  string `json:"type"`
	TTL   int64  `json:"ttl"`
...
type redisImportRequest struct {
	Items   []redisExportItem `json:"items"`
	Confirm bool              `json:"confirm"`
}

type redisZSetExportMember struct {
	Member string  `json:"member"`
	Score  float64 `json:"score"`
}

// ExportKeys exports Redis keys matching a pattern as a JSON snapshot.
func (h RedisHandler) ExportKeys(w http.ResponseWriter, r *http.Request) {
	connID := router.Param(r, "id")
	dbIndex, err := dbIndexParam(r)
	if err != nil {
		logWriteErr(h.Logger, contract.WriteError(w, r, contract.NewErrorBuilder().
	...
	limit := parseBoundedLimit(r.URL.Query().Get("limit"), DefaultRedisExportKeys, MaxRedisExportKeys)
	...
	if h.Registry != nil {
		...
		defer h.Registry.Unregister(operationID)
	}

	payload := redisExportPayload{Version: "dbadmin.redis.v1", DB: dbIndex, Pattern: pattern, Limit: limit}
	var cursor uint64
	for {
		keys, nextCursor, err := cl.Scan(ctx, cursor, pattern, 100).Result()
		if err != nil {
			h.internalErr(w, r, err)
			return
		}
		for _, key := range keys {
			...
			item, err := h.redisExportItem(ctx, cl, key)
			if err != nil {
				h.Logger.Warn("skip redis export key", plumelog.Fields{"key": key, "error": err.Error()})
				continue
			}
			payload.Items = append(payload.Items, item)
		}
		if payload.Truncated || nextCursor == 0 {
			break
		}
		cursor = nextCursor
	}
	...
	w.Header().Set("Content-Disposition", fmt.Sprintf(`attachment; filename="redis_db%d_%s.json"`, dbIndex, time.Now().Format("20060102_150405")))
...
func (h RedisHandler) redisExportItem(ctx context.Context, cl *redis.Client, key string) (redisExportItem, error) {
	...
	ttlSec := int64(-1)
	if ttl, err := cl.TTL(ctx, key).Result(); err == nil {
	...
	item := redisExportItem{Key: key, Type: keyType, TTL: ttlSec}
	switch keyType {
	case "string":
		item.Value, err = cl.Get(ctx, key).Result()
	case "hash":
		item.Value, err = cl.HGetAll(ctx, key).Result()
	case "list":
		item.Value, err = cl.LRange(ctx, key, 0, MaxRedisContainerEntries-1).Result()
	case "set":
		item.Value, err = cl.SMembers(ctx, key).Result()
	case "zset":
		members, zerr := cl.ZRangeWithScores(ctx, key, 0, MaxRedisContainerEntries-1).Result()
		if zerr != nil {
			return item, zerr
		}
		out := make([]redisZSetExportMember, 0, len(members))
		for _, m := range members {
			out = append(out, redisZSetExportMember{Member: fmt.Sprint(m.Member), Score: m.Score})
		}
		item.Value = out
	case "stream":
		messages, zerr := cl.XRangeN(ctx, key, "-", "+", MaxRedisContainerEntries).Result()
		if zerr != nil {
			return item, zerr
		}
		item.Value = messages
...
// ImportKeys restores keys from a JSON snapshot generated by ExportKeys.
func (h RedisHandler) ImportKeys(w http.ResponseWriter, r *http.Request) {
	connID := router.Param(r, "id")
	dbIndex, err := dbIndexParam(r)
	if err != nil {
		logWriteErr(h.Logger, contract.WriteError(w, r, contract.NewErrorBuilder().
	...
	var req redisImportRequest
	if !decodeJSONLimited(w, r, h.Logger, &req) {
		return
	}
	if !req.Confirm {
		logWriteErr(h.Logger, contract.WriteError(w, r, contract.NewErrorBuilder().
			Type(contract.TypeBadRequest).Message("confirm=true required").Build()))
		return
	}
	if len(req.Items) == 0 {
		logWriteErr(h.Logger, contract.WriteError(w, r, contract.NewErrorBuilder().
			Type(contract.TypeBadRequest).Message("no keys to import").Build()))
		return
	}
	if len(req.Items) > MaxRedisExportKeys {
		logWriteErr(h.Logger, contract.WriteError(w, r, contract.NewErrorBuilder().
			Type(contract.TypeBadRequest).Message("import is limited to "+strconv.Itoa(MaxRedisExportKeys)+" keys").Build()))
		return
	}
	...
	defer cancel()
	imported := 0
	var errorsDetail []importErrorDetail
	for i, item := range req.Items {
		if err := importRedisItem(ctx, cl, item); err != nil {
			errorsDetail = append(errorsDetail, importErrorDetail{Index: i + 1, Snippet: truncate(item.Key, 120), Error: err.Error()})
			continue
		}
		imported++
	}
...
func importRedisItem(ctx context.Context, cl *redis.Client, item redisExportItem) error {
	...
	if err := cl.Del(ctx, item.Key).Err(); err != nil {
		return err
	}
	switch item.Type {
	case "string":
		if err := cl.Set(ctx, item.Key, fmt.Sprint(item.Value), 0).Err(); err != nil {
			return err
		}
	case "hash":
		values, err := stringMap(item.Value)
		if err != nil {
			return err
		}
		if len(values) > 0 {
			if err := cl.HSet(ctx, item.Key, values).Err(); err != nil {
				return err
			}
		}
	case "list":
		values := stringSlice(item.Value)
		if len(values) > 0 {
			if err := cl.RPush(ctx, item.Key, values...).Err(); err != nil {
				return err
			}
		}
	case "set":
		values := stringSlice(item.Value)
		if len(values) > 0 {
			if err := cl.SAdd(ctx, item.Key, values...).Err(); err != nil {
				return err
			}
		}
	case "zset":
		values, err := zsetMembers(item.Value)
		if err != nil {
			return err
		}
		if len(values) > 0 {
			if err := cl.ZAdd(ctx, item.Key, values...).Err(); err != nil {
				return err
			}
		}
	default:
		return fmt.Errorf("unsupported redis type %q", item.Type)
	}
	if item.TTL > 0 {
		return cl.Expire(ctx, item.Key, time.Duration(item.TTL)*time.Second).Err()
	}
	return nil
}
...
func stringMap(value any) (map[string]string, error) {
	...
	if !ok {
		return nil, fmt.Errorf("hash value must be an object")
	}
...
func zsetMembers(value any) ([]redis.Z, error) {
	raw, ok := value.([]any)
	if !ok {
		return nil, fmt.Errorf("zset value must be an array")
	}
	out := make([]redis.Z, 0, len(raw))
	for _, v := range raw {
		item, ok := v.(map[string]any)
		if !ok {
			return nil, fmt.Errorf("zset member must be an object")
		}
		score, ok := item["score"].(float64)
		if !ok {
			return nil, fmt.Errorf("zset score is required")
		}
		out = append(out, redis.Z{Member: fmt.Sprint(item["member"]), Score: score})
	}
	return out, nil
}
