...
func (h *Handler) ListWorkers(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "LIST_WORKERS_NOT_CONFIGURED", "list workers service not configured")
		return
	}

	query, ok := h.parseWorkerListQuery(w, r)
	if !ok {
		return
	}
...
func (h *Handler) GetWorker(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "GET_WORKER_NOT_CONFIGURED", "get worker service not configured")
		return
	}

	workerID := strings.TrimSpace(router.Param(r, "worker_id"))
	if workerID == "" {
		h.writeRequiredPathParam(w, r, "worker_id")
		return
	}
...
func (h *Handler) GetTask(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "GET_TASK_NOT_CONFIGURED", "get task service not configured")
		return
	}

	taskID := strings.TrimSpace(router.Param(r, "task_id"))
	if taskID == "" {
		h.writeRequiredPathParam(w, r, "task_id")
		return
	}
...
func (h *Handler) GetCaseTimeline(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "GET_CASE_TIMELINE_NOT_CONFIGURED", "get case timeline service not configured")
		return
	}
...
func (h *Handler) ListExecPlanCases(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "LIST_EXEC_PLAN_CASES_NOT_CONFIGURED", "list exec plan cases service not configured")
		return
	}

	execPlanID := strings.TrimSpace(router.Param(r, "exec_plan_id"))
	if execPlanID == "" {
		h.writeRequiredPathParam(w, r, "exec_plan_id")
		return
	}
...
func (h *Handler) FleetSummary(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "FLEET_SUMMARY_NOT_CONFIGURED", "fleet summary service not configured")
		return
	}
...
func (h *Handler) ListAlerts(w http.ResponseWriter, r *http.Request) {
	...
	if h.service == nil {
		h.writeNotImplemented(w, r, "LIST_ALERTS_NOT_CONFIGURED", "list alerts service not configured")
		return
	}
...
func (h *Handler) parseWorkerListQuery(w http.ResponseWriter, r *http.Request) (workerapp.WorkerListQuery, bool) {
	page, pageSize, ok := h.parsePagination(w, r)
	if !ok {
	...
	if err != nil {
		h.writeInvalidQuery(w, r, "accepting_tasks", "accepting_tasks must be a boolean")
		return workerapp.WorkerListQuery{}, false
	}
...
func (h *Handler) parsePagination(w http.ResponseWriter, r *http.Request) (int, int, bool) {
	page := 1
	pageSize := 50
	...
	if rawPage := strings.TrimSpace(r.URL.Query().Get("page")); rawPage != "" {
		...
		if err != nil || value < 1 {
			h.writeInvalidQuery(w, r, "page", "page must be a positive integer")
			return 0, 0, false
		}
		page = value
	}
	...
	if rawPageSize := strings.TrimSpace(r.URL.Query().Get("page_size")); rawPageSize != "" {
		...
		if err != nil || value < 1 || value > 500 {
			h.writeInvalidQuery(w, r, "page_size", "page_size must be between 1 and 500")
			return 0, 0, false
		}
		pageSize = value
	}
...
func (h *Handler) writeInvalidQuery(w http.ResponseWriter, r *http.Request, field string, message string) {
...
func (h *Handler) writeRequiredPathParam(w http.ResponseWriter, r *http.Request, field string) {
	logWriteErr(h.Logger, contract.WriteError(w, r, contract.NewErrorBuilder().
