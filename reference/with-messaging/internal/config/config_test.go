...
func TestLoadPrecedenceDefaultsEnvFileEnvironmentFlags(t *testing.T) {
	...
	cfg, err := load(
		[]string{"with-messaging", "--env-file", envFile, "--addr", ":9000", "--debug=false"},
...
func TestLoadEnvironmentOverridesEnvFile(t *testing.T) {
	...
	cfg, err := load(
		[]string{"with-messaging", "--env-file=" + envFile},
		mapLookup(map[string]string{
			"APP_ADDR":  ":8000",
			"APP_DEBUG": "true",
		}),
...
func TestLoadIgnoresUnrelatedFlags(t *testing.T) {
	cfg, err := load(
		[]string{"with-messaging", "--output", "openapi.json", "--format=json", "--addr", ":9000"},
		mapLookup(nil),
	)
	if err != nil {
		t.Fatalf("load config: %v", err)
