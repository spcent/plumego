...
provide dependency-free RPC lifecycle abstractions, with offline tests and a
hello-world concrete gRPC reference app.
Constraints: no `go.mod` may exist under `x/**`, no gRPC import in main module
or stable roots, concrete gRPC dependencies stay in `reference/with-rpc`, and
protobuf code generation is the user's responsibility.
...
|------|------|----------------|-------------|------------|-------------|
| 1570 | Create x/rpc/server/ wrapping caller-owned runtime lifecycle | x/rpc | `x/rpc/server/server.go`, `x/rpc/server/server_test.go`, `x/rpc/module.yaml` | M-014 | `go test ./x/rpc/server/...`, `go vet ./x/rpc/...` |
| 1571 | Create x/rpc/client/ with Pool and dependency-free interceptors | x/rpc | `x/rpc/client/pool.go`, `x/rpc/client/pool_test.go` | 1570 | `go test ./x/rpc/client/...` |
| 1572 | Create x/rpc/gateway/ with Transcoder registration primitives | x/rpc | `x/rpc/gateway/transcoder.go`, `x/rpc/gateway/transcoder_test.go` | 1570 | `go test ./x/rpc/gateway/...` |
...
- Group A: card 1570 — must complete first; creates the server package that
  establishes the runtime boundary.
- Group B (parallel after A): cards 1571 and 1572 — independent sub-packages sharing
  the dependency-free runtime contracts created in 1570.
- Group C (sequential): reference/with-rpc and acceptance criteria after all three cards.

## Risk Register

- Risk: concrete gRPC dependencies leak into x/rpc.
  Mitigation: keep x/rpc dependency-free and confine grpc/grpc-gateway imports
  to `reference/with-rpc`.
