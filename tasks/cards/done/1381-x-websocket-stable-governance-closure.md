...
- Generate release-to-release API snapshots rather than current-head-only
  snapshots.
- Record `realtime` owner sign-off.
- Keep `x/websocket` out of beta until evidence is complete.
...
Outcome:
- Release refs `d2c25c3` and `ec70358`, matching API snapshots, and
  `realtime` owner sign-off are checked in.
- Evidence and maturity checks pass with `x/websocket` as beta.
