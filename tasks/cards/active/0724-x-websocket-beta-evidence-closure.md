...
Problem:
The evidence ledger has an `x/websocket` evidence doc and current-head snapshot, but beta promotion remains blocked by missing release history, matching release snapshots, and owner sign-off.
...
Done Definition:
- `x/websocket` has two release refs, matching release snapshots, and owner sign-off, or the blocker remains explicit.
