...
- `Server()` returns the prepared `*http.Server` for `net/http` compatibility. If caller code replaces fields such as `Handler`, `ConnState`, `TLSConfig`, or `TLSNextProto`, that override is caller-owned and can bypass core middleware, active-connection tracking, loaded TLS material, or HTTP/2 policy.
