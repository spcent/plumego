...
- **可靠队列/持久化消息处理**：已新增 `net/mq` SQL 持久化可靠队列（租约、重试、DLQ、延迟），可用于短信发送与重试调度；但仍缺少分布式集群/跨进程协调的 MQ 能力。
...
- `tenant`：租户解析、配额、策略控制。
- `net/mq`：已具备 SQL 持久化可靠队列（租约、重试、DLQ、延迟），用于发送队列与重试调度。
- `pubsub/scheduler`：仍可用于进程内事件扇出与轻量调度（非持久化）。
