...
Plumego is a lightweight Go HTTP toolkit built entirely on the standard library. It covers routing, middleware, graceful shutdown, WebSocket utilities, webhook pipelines, and static frontend serving. It's designed to be embedded into your own `main` package, not as a standalone framework binary.
...
- **Middleware Chain**: Logging, recovery, gzip, CORS, timeout, rate limiting, concurrency limiting, request body size limits, and authentication helpers, all wrapping standard `http.Handler`.
