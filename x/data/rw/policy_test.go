...
func TestSQLTypePolicy(t *testing.T) {
	...
	tests := []struct {
		...
		{"SELECT literal containing FOR UPDATE", "SELECT 'FOR UPDATE' AS label FROM users", false},
		{"SELECT literal containing FOR SHARE", "SELECT * FROM users WHERE note = 'FOR SHARE'", false},
		{"SELECT comment containing FOR UPDATE", "SELECT * FROM users -- FOR UPDATE\nWHERE id = 1", false},
...
func TestIsWriteOperation(t *testing.T) {
	tests := []struct {
		...
		{"SELECT 'FOR UPDATE' AS lock_label", false},
		{"SELECT * FROM users /* FOR SHARE */ WHERE id = 1", false},
		{"-- comment\nINSERT INTO users VALUES (1)", true},
