...
func TestPubSub_MultiSubscriberReceive(t *testing.T) {
	...
	sub1, err := ps.Subscribe(context.Background(), "user.created", SubOptions{BufferSize: 8, Policy: DropOldest})
	if err != nil {
		t.Fatalf("subscribe sub1: %v", err)
	}
	sub2, err := ps.Subscribe(context.Background(), "user.created", SubOptions{BufferSize: 8, Policy: DropOldest})
	if err != nil {
...
func TestPubSub_CancelStopsDelivery(t *testing.T) {
	...
	sub, err := ps.Subscribe(context.Background(), "t", SubOptions{BufferSize: 1, Policy: DropOldest})
	if err != nil {
...
func TestPubSub_MessageImmutability(t *testing.T) {
	...
	sub1, err := ps.Subscribe(context.Background(), "topic", SubOptions{BufferSize: 4, Policy: DropOldest})
	if err != nil {
		t.Fatalf("subscribe sub1: %v", err)
	}
	sub2, err := ps.Subscribe(context.Background(), "topic", SubOptions{BufferSize: 4, Policy: DropOldest})
	if err != nil {
		t.Fatalf("subscribe sub2: %v", err)
	}
